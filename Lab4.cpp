...
// Функция для поиска двух подряд идущих чисел степени двойки, где второе больше первого
void findPowersOfTwo(int arr[]) {
    bool found = false;
    
    for (int i = 0; i < SIZE - 1; i++) {
        if (isPowerOfTwo(arr[i]) && isPowerOfTwo(arr[i + 1])) {
            if (arr[i + 1] > arr[i]) {
                cout << "Найдено начало последовательности степеней двойки:\t";
                cout << "arr[" << i << "] = " << arr[i] << " и arr[" << i + 1 << "] = " << arr[i + 1] << endl;
                cout << "Номер элемента c которого начинается последовательность: " << i << endl;
                found = true;
                break;
            }
        }
